...
export function phaseOne(game, coord, handler, phaseText) {
...
export function phaseTwo(game, coord, handler, phaseText) {
    if (typeof game.Selected === 'undefined') {
        if (game.RemovingAPiece) {
            ...
            phaseTwoSelect(game, coord, handler);
        }
    }
    else {
...
function phaseTwoSelect(game, coord, handler) {
...
function phaseTwoMove(game, coord, handler, phaseText) {
    if (game.isValidMovePhaseOne(coord)) {
        let element = document.createElement("i");
        element.style.fontSize = "40px";
        element.classList.add("material-icons");
...
export function hoverAction(handler, game, out) {
    let color1 = "#a1887f";
    let color2 = "#a1887f";
    if (out) {
        color2 = "#8d6e63";
